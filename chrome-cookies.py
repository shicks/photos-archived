...
def chrome_cookies():
    ...
    if sys.platform == 'linux2':
        ...
        cookie_file = os.path.expanduser(
            ...
            # TODO(sdh): make this configurable...
        )
    else:
    ...
    #if url: domain = urllib.parse.urlparse(url).netloc
    #domain_no_sub = '.'.join(domain.split('.')[-2:])
 
    conn = sqlite3.connect(cookie_file)
    sql = 'select host_key, path, secure, expires_utc, name, value, encrypted_value from cookies '#\
    ...
    # with conn:
    #     for hk, p, s, exp, k, v, ev in conn.execute(sql):
    ...
    # print the following:
    # hk \t (hk startswith . ? true : false) \t p \t (s ? true : false)
    #    \t exp \t name \t value

    with conn:
        for hk, p, s, exp, k, v, ev in conn.execute(sql):
 
            if not v and (ev[:3] == b'v10'):
                v = chrome_decrypt(ev, key=key)

            print "\t".join(['%s']*7) % (
                hk,
                'true' if hk.startswith('.') else 'false',
                p,
